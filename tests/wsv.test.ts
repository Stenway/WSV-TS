...
function splitBytes(bytes: Uint8Array, splitByte: number, removeEmpty: boolean): Uint8Array[] {
	...
	for (;;) {
		...
		if (currentIndex < 0) {
			...
			if (!(part.length === 0 && removeEmpty === true)) { result.push(part) }
			break
		} else {
...
function decodeValue(bytes: Uint8Array): string | null {
	if (bytes.length === 0) { throw new Error(`Invalid WSV value byte sequence`) }
	if (bytes.length === 1) {
		const firstByte = bytes[0]
		if (firstByte === BinaryWsvUtil.nullValueByte) { return null }
		else if (firstByte === BinaryWsvUtil.emptyStringByte) { return "" }
	}
...
describe("BinaryWsvEncoder.encode + BinaryWsvDecoder.decode + ...", () => {
	test.each([
		...
		(input) => {
			...
			const jaggedArrayBySplit = bytesOfLines.map((lineBytes) => 
				splitBytes(lineBytes, valueSeparatorByte, true).map((valueBytes) => decodeValue(valueBytes))
			)
