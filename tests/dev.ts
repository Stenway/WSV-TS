...
function decodeValue(bytes: Uint8Array): string | null {
	...
	if (bytes.length === 1) {
		...
		if (firstByte === nullValueByte) { return null }
		else if (firstByte === emptyStringByte) { return "" }
	}
	const decoder = new TextDecoder("utf-8", {fatal: true, ignoreBOM: true})
	return decoder.decode(bytes)
}

function decodeBinaryWsvDocument(bytes: Uint8Array): (string | null)[][] {
	return splitBytes(bytes.subarray(3), lineBreakByte, false)
		.map((lineBytes) => 
			splitBytes(lineBytes, valueSeparatorByte, true)
